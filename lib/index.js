...
export async function exec (argv2) { // eslint-disable-line complexity
  const argv = minimist(argv2, {
    boolean: [ 'b', 'build', 'bytecode', 'd', 'debug',
      'h', 'help', 'public', 'v', 'version', 'externalModules' ],
  ...
  if (argv.externalModules === true) {
    log.debug('Require of external modules will be allowed..');
  ...
  for (const target of targets) {
    ...
    const vfsOutput = argv.vfsOutput ? path.resolve(argv.vfsOutput) : undefined;
    const vfsFullOutput = argv.vfsFullOutput ? path.resolve(argv.vfsFullOutput) : undefined;
    const slash = target.platform === 'win' ? '\\' : '/';
    await producer({ backpack, vfsOutput, vfsFullOutput, bakes, baseDir, slash, target });
