...
export default function ({ backpack, bakes, vfsFullOutput, vfsOutput, baseDir, slash, target }) {
  return new Promise((resolve, reject) => {
    ...
    if (baseDir) {
      baseDirPath = snapshotify(baseDir, slash);
    } else {
      baseDirPath = '';
    }
    ...
    multistream((cb) => {
            ...
            intoStream(makePreludeBufferFromPrelude(
              replaceDollarWise(
                ...
                replaceDollarWise(prelude, '%VIRTUAL_FILESYSTEM%', JSON.stringify(vfs)),
                                           ...
                                           '%BASEDIR%', JSON.stringify(baseDirPath))
            ))
          ));
        }
      } else {
      ...
      if (vfsFullOutput || vfsOutput) {
        ...
        fs.writeFileSync(vfsTargetPath, JSON.stringify(content, null, 2));
      }
